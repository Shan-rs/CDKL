...
class DehazingSet(data.Dataset):
    def __init__(self, root, aug, is_train): 
        ...
        self.label_path = root + '/gt/' 
        hazy_imgs = os.listdir(root + '/hazy/')#
        self.hazy_imgs = [root + '/hazy/' + img for img in hazy_imgs]
        self.hazy_imgs.sort()
        self.transform = T.Compose([T.Resize(256), T.ToTensor()]) #
        self.transformsal = T.Compose([T.Resize(256), T.ToTensor(), ThresholdTransform(th=0.9)])#T.Grayscale(num_output_channels=1),
...
#        gt_path = self.gt_imgs_path + hazy_path.split('/')[-1]
        labelname = hazy_path.split('/')[-1][:-6] + '.jpg'
        label_path = self.label_path+ labelname
        hazy_im = Image.open(hazy_path)
        gt_img = Image.open(gt_path)
        label = Image.open(label_path).convert('L')
        
        if self.is_train:
            i,j,h,w=T.RandomCrop.get_params(hazy_im, output_size=(256, 256))#384 224
        ...
        if self.aug:
            ...
            clear = self.transform(flip_rot(gt_img))
            label = self.transformsal(flip_rot(label))
        else:
            hazy_img = self.transform(hazy_im)
            clear = self.transform(gt_img)
            label = self.transformsal(label)
        return hazy_img, clear, label, labelname
