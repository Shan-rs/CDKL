...
class REC_dehaze(nn.Module):
    def __init__(self, num_init_features, out_c=12):
        super(REC_dehaze, self).__init__()
...
class ContextAwareAggregation(nn.Module):
    def __init__(self, l_inchannel):
        super(ContextAwareAggregation, self).__init__()
        self.conv1 = nn.Conv2d(2*l_inchannel, l_inchannel, kernel_size=1, stride=1, padding=0, bias=True)
        self.relu = nn.ReLU(inplace=True)
        self.ca = CALayer(l_inchannel)
        self.pa = PALayer(l_inchannel)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.softmax = nn.Softmax(dim=1)
        self.post_precess = nn.Sequential(
                nn.Conv2d(l_inchannel, l_inchannel, 3, padding=1, bias=True),
                nn.Conv2d(l_inchannel, l_inchannel, 3, padding=1, bias=True),
        )

    def forward(self, x, y): #x smaller than y
        x = self.relu(self.conv1(x))
        x1 = self.ca(x)
        x1 = self.pa(x1)
        x1 = x1 + x
        x1  = F.interpolate(x1, size=y.size()[2:], mode='bilinear',align_corners=True)
        w1 = self.softmax(self.avg_pool(x))
        
        y1 = self.ca(y)
        y1 = self.pa(y1)
        y1 = y1 + y
        w2 = self.softmax(self.avg_pool(y))
        
        out = self.relu(w1*x1 + w2*y1)
        out = self.post_precess(out)
        return out

if __name__ == "__main__":
#    model1 = SpectrumAwareAggregation()
    model1 = DynamicMutualEnhancement(256)
    
    x = torch.randn(4, 256, 128, 128)
    y = torch.randn(4, 512, 64, 64)
    z = torch.randn(4, 3, 214, 224)
    out1 = model1(x, y)
    print(out1.size())
#    z = torch.randn([1, 3, 51, 64])
#    bl, cl, wl, hl = z.size()
#    down = nn.Conv2d(3, 6, 3, stride = 2, padding=1, bias=True)
